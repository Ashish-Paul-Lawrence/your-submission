...
def analyze(lines, encode):
    ...
    """
def analyze(lines, encode):
    #Return corpus-level (fertility, tokens_per_char).
    ...
    tok_per_sentence = total_tokens / len(lines) if len(lines) else 0 

    return fertility, tokens_per_char, tok_per_sentence
...
def main():
    ...
    print(f"{'lang':<8}{'fertility (tok/word)':>22}{'tok/char':>12}{'tok/sentence':>18}")
    print("-" * 42)
    results = {}
    for spec in args.corpus:
        lang, path = spec.split("=", 1)
        lines = read_lines(path)
        fert, tpc, tps = analyze(lines, encode)
        results[lang] = (fert, tpc, tps)
        print(f"{lang:<8}{fert:>22.2f}{tpc:>12.3f}{tps:>12.3f}")
    ...
    if len(results) >= 2:
        ...
        print()
        for lang in langs[1:]:
            ratio = results[lang][2] / results[base][2]
